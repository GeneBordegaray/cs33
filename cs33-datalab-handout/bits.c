 ...
 */
int tmax(void) {
       int numMax = ~(1 << 31); //store the complement of shifting 1 left 31 times bc ints are signed by default
       return numMax; //return numMax
}
/*
 ...
 */
int isZero(int x) {
  int result = !(x ^ 0); //oppposite of xor comparision between parameter and 0
  return result; 
}
/* 
...
int bitXor(int x, int y) {
  ...
  //what we currently have is the complement to what bitxor would return
  int finalResult = (~firstResult & ~secondResult); //take complement of & comparison between the first two results
  return finalResult;
}
/* 
...
int isNotEqual(int x, int y) {
  ...
  int finalResult = !(secondResult); //use the not operator, should hold 0 if x and y are equal and 1 vice versa
  return finalResult;
}
/* 
